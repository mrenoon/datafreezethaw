...
class FreezeThawGP(BaseModel):
    ...
    def pred_old_all(self, conf_nr, steps, fro=None):
        ...
        if self.activated:
            ...
            std2 -= mean**2
            if VU_PRINT >= -4:
                print "         :::::::       PREDICTING FOR OLD CONFIG: ", self.X[conf_nr-1], "   =>  mean = ", mean, " std2 = ", std2
    ...
    def pred_new(self, step, asy_mean, y=None):
        ...
        if VU_PRINT >= -1:
            print "===========  in GP.pred_new, k_x_x = ", k_x_x, " k_x_x + noise = ", k_x_x + self.noiseCurve * np.eye(k_x_x.shape[0])
        ...
        if VU_PRINT >= -1:
            print "===========  in GP.pred_new, tprime =  ", tprime, " k_xstar_xstar = ", k_xstar_xstar, " k_xstar_x = ", k_xstar_x
    ...
    def pred_new_all(self, steps=13, xprime=None, y=None, asy=False):
        ...
        #     print "in GP.pred_new_all(), the end result : mean = ", mean, " std2 = ", std2
        if VU_PRINT >= -4:
            # print "in GP.pred_new_all(), after calculating all the GP hypers, mean_temp = ", mean_temp
            # print "in GP.pred_new_all(), after calculating all the GP hypers, std2_temp = ", std2_temp
