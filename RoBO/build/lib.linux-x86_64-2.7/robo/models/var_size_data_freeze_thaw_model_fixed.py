...
class VarSizeDataFreezeThawGP(BaseModel):
    ...
    def train(self, X=None, Y=None, S=None, do_optimize=True):
        ...
        if do_optimize:
            ...
            self.printAlphaBetaTheta()

    
    def predict(self, xprime=None, option='asympt', datasize=None, conf_nr=0, from_step=None, further_steps=1, full_cov=False):
        """
        ...
        if option == 'asympt':
            ...
            if from_step is None:
                ...
                mu, std2, _ = self.pred_old_all(
                    conf_nr=conf_nr + 1, steps=further_steps, datasize=datasize)
            else:
                mu, std2, _ = self.pred_old_all(
    ...
    def pred_asympt_all(self, xprime, full_cov=False):
        ...
        if not full_cov:
            ...
            for i in xrange(self.samples.shape[0]):
                ...
                pre = self.pred_asympt(xprime,show=False)
            ...
            for i in xrange(self.samples.shape[0]):
                ...
                if pre is not None:
                    ...
                    divby -= 1
                    print 'in pred_asympt_all, bad !!, divby = ', divby

            mean_temp = np.zeros((divby, xprime.shape[0]))
            std2_temp = np.zeros((divby, xprime.shape[0]))
            
            if VU_PRINT >= -4:
                # print "in pred_asympt_all, cov_val = ", cov_val
                print "self.samples.shape = ", self.samples.shape
                print "cov_val.len = ", len(cov_val)
    ...
    def printAlphaBetaTheta(self):
        flex = self.X.shape[-1]

        alphasSum = 0
        betasSum = 0
        thetadSum = np.zeros(flex)
        divby = self.samples.shape[0]
        for i in range(divby):
            alphasSum += self.samples[i][flex+1]
            betasSum += self.samples[i][flex+2]
            for j in range(flex):
                thetadSum[j] += self.samples[i][j]

        print "             ========================================================================== "
        print "             FYI: beta = ", betasSum/divby , ", alpha =", alphasSum/divby, " thetad=", thetadSum/divby
        print "             ========================================================================== "

    #DONE, I think
    def pred_old(self, s, sprime, yn, mu_n=None, Cnn=None):
        # if VU_PRINT >= -5:
        #     print "                                           FYI: self.beta = ",self.beta, ", self.alpha =",self.alpha

        if VU_PRINT >= -4:
            print "pred_old, s = ", s , " sprime = ", sprime

        yn = yn.reshape(-1, 1)

        ksn = self.kernel_curve_s(s, s)
        if ksn is None:
            return None


        ksn_inv = self.invers(ksn)
        if ksn_inv is None:
            return None

        ksn_star = self.kernel_curve_s(s, sprime)
        if ksn_star is None:
            return None
        ...
        Omega = np.ones((sprime.shape[0], 1)) - np.dot(ksn_star.T,
                                                       np.dot(ksn_inv, np.ones((s.shape[0], 1))))

        # Exactly why:
        if yn.shape[0] > ksn_inv.shape[0]:
            yn = yn[:ksn_inv.shape[0]]

        if VU_PRINT >= -4:
            print "                             FYI: ksn in pred_old is: ", ksn
            print "                             FYI: ksn_star in pred_old is: ", ksn_star
            print "                             FYI: yn = ", yn
            print "                             FYI: mu_n = ", mu_n


        mean = np.dot(ksn_star.T, np.dot(ksn_inv, yn)) + Omega*mu_n

        ksn_star_star = self.kernel_curve_s(sprime, sprime)

        if ksn_star_star is None:
            return None

        cov = ksn_star_star - \
            np.dot(ksn_star.T, np.dot(ksn_inv, ksn_star)) + \
    ...
    #DONE I think
    def pred_old_all(self, conf_nr, steps, fro=None, datasize=None):
        ...
        if self.activated:
            ...
            if fro is None:
                ...
                s = self.s_train[conf_nr -1]
                
                # tprime = np.arange(yn.shape[0] + 1, yn.shape[0] + 1 + steps)
                sprime = np.array([datasize,])

            else:
                assert 1>2, "Not supposed to be here, pred_old_all"
                t = np.arange(1, fro)
                tprime = np.arange(fro, fro + steps)

            if VU_PRINT >= -5:
                print "|||||||||        PREDICTING OLD, x = ", self.X[conf_nr-1], "- #", conf_nr-1  #, " fro = ", fro, " is activated ?", self.activated
                # print "      :::::::            s = ", s, " , sprime = ", sprime
            ...
            for i in xrange(self.samples.shape[0]):
                ...
                pre = self.pred_old(s, sprime, yn, mu_n, Cnn)
            ...
            if VU_PRINT >= -5:
                print "|||||||||        Result, last= ", self.y_train[conf_nr-1], "=> mean = ", mean, " std2 = ", std2, ", asympt=", sum_mu/self.samples.shape[0]
                # print ""
    ...
    #DONE
    def pred_new_new(self, step, asy_mean, asy_std2, datasize):
        ...
        # fro = 1
        s = np.array(datasize)
        k_s_s = self.kernel_curve_s(s,s)

        # if VU_PRINT >= -4:
        #     print "===========  in GP.pred_new, return mean = ", asy_mean, " std2 = ", asy_std2 + k_ts_ts

        #Why is the mean the same as asy_mean ??? Is it really so ?? Yeah it is I guess


        return asy_mean, asy_std2 + k_s_s
    ...
    def pred_new_all(self, steps=13, xprime=None, y=None, asy=False):
        ...
        if asy is False:
            ...
            asy_mean, std2star, _ = self.pred_asympt_all(realXprime)
            if VU_PRINT>=-4:
                print "in GP.pred_new_all, realXprime = ", realXprime, "realXprime=", realXprime
                print "in GP.pred_new_all, datasize = ", datasize
        ...
        for i in xrange(self.samples.shape[0]):
            ...
            mean_one, std2_one = self.pred_new_new(
                steps, asy_mean, std2star, datasize)
    ...
    def gammaLambda(self, m_const):
        ...
        for idex, yn in enumerate(self.ys):
            yn = yn.reshape(-1,1)
            ...
            if ksn_inv is None:
                return None
            one_n = np.ones((ksn.shape[0], 1))
            onenT_ksnInv = np.dot(one_n.T, ksn_inv)

            Lambda[index, index] = np.dot(onenT_ksnInv, one_n)
            gamma[index, 0] = np.dot(onenT_ksnInv, yn - m_const[index])            
