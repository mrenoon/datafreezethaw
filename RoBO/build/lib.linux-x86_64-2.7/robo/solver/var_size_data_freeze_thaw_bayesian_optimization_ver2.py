...
from robo.acquisition.information_gain_mc_data_freeze import InformationGainMC, get_training_time
    ...
    def run(self, num_iterations=10, X=None, Y=None):
        ...
        for k in range(self.init_points, num_iterations):
            ...
            for i in xrange(nr_new):
                IG_new[i] = self.ig.compute(np.array([ newConfigs[i] ]), time_overhead)
            ...
            for i in xrange(nr_old):
                ...
                IG_old[i], newSize[i] = self.ig.computeOldModel(config_id, time_overhead)
            ...
            oldWins = True
            if IG_old[winnerOld] < IG_new[winnerNew]:
