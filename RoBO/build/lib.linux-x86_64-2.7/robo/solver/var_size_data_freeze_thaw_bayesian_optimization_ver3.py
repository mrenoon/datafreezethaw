...
class VarSizeDataFreezeThawBO(BaseSolver):
    ...
    def run(self, num_iterations=10, X=None, Y=None):
        ...
        for i in range(self.init_points):
            self.basketOld_S[i] = np.asarray([0.04])
        ...
        self.maximizer = Direct(self.ig, self.task.X_lower, self.task.X_upper, n_func_evals=200, verbose=False)
        ...
        for k in range(self.init_points, num_iterations):
            ...
            for i in xrange(nr_old):
                ...
                IG_old[i], newSize[i] = self.ig.computeOldModel(config_id)
            ...
            if VU_PRINT >= 0:
                print "$$$$$$$$  IG/unit time of new candidate = ", newConfigIG
                print ""

            winnerOld = np.argmax(IG_old)
            oldWins = True
            if IG_old[winnerOld] < newConfigIG:
                print "Winner: the new config"
                oldWins = False
            else:
            ...
            if VU_PRINT >= -10:
                print "%%%%%%%% TIME OVERHEAD = ", time_overhead
            ...
            if oldWins:
                ...
                if VU_PRINT >= -10:
                    print "%%%%%%%% TIME FOR EVALUATION = ", time_func_eval, " s = ", stplus1 * 500, " TOTAL=", time_overhead + time_func_eval, " (estimated=",self.ig.get_training_time(stplus1)
    ...
    def choose_next_ei(self, X=None, Y=None, S=None, do_optimize=True, N=10, M=3):
        """
        Recommend a new configuration by maximizing the acquisition function
        ...
        for i in xrange(N):
            ei_list[i] = compute_ei(X=x[i,:], model=self.freezeModel, ys=Y)
...
def compute_ei(X, model, ys, par=0.0):
    m, v = model.predict(X[None,:-1])

    Y = getY(ys)

    eta = estimate_incumbent(Y, X)
    # eta = eta[0,0]
