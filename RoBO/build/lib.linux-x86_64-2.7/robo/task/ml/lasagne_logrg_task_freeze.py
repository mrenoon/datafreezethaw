...
class LogisticRegression(BaseTask):
    ...
    def set_weights(self, old_file_name):   # FREEZE
        print("             IN set_weights !!!!!!!!!!!!!!!!!!!!!!!! " )
        file_name = old_file_name + '.npz'
        with np.load(file_name) as f:
            param_values = [f['arr_%d' % i] for i in range(len(f.files))]

        if VU_PRINT:
            pprint("##############################    in lasagne_logrg_task_freeze, set_weights, param_values =  ")
            pprint(param_values)
        lasagne.layers.set_all_param_values(self.network, param_values)

    def set_epochs(self, n_epochs):     # FREEZE
        self.num_epochs = n_epochs

    def set_save_modus(self, is_old=True, file_old=None, file_new=None):    # FREEZE
    ...
    def objective_function(self, x):
        ...
        print "After RESULT: self.save = " , self.save

        if self.save:
            if VU_PRINT:
                pprint("-----------------------------    in lasagne_logrg_task_freeze, objective_function , param_values after training =  ")
                pprint(lasagne.layers.get_all_param_values(self.network))
            print("in self.save")
            file_name = self.file_name + '.npz'
            np.savez(file_name, *lasagne.layers.get_all_param_values(self.network))
        else:
            print("self.save not run")
