...
class InformationGainPerUnitCost(InformationGain):
    ...
    def update(self, model, cost_model=None, overhead=None):
        # self.cost_model = cost_model
        if overhead is None:
            self.overhead = 0
        else:
    ...
    def compute(self, X, derivative=False):
        ...
        cost = self.get_training_time(X[-1]) + self.overhead
    ...
    def get_training_time(self, S):
        result = np.exp(S/20)
        if VU_PRINT >= 1:
            print "ESTIMATE training time for dataset size = ", S, " = ", result
        return result
