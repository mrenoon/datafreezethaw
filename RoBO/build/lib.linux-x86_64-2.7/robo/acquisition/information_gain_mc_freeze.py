...
class InformationGainMC(BaseAcquisitionFunction):
    ...
    def compute(self, X=None, derivative=False, *args):
    ...
    def sampling_acquisition_wrapper(self, x):
        if np.any(x < self.X_lower) or np.any(x > self.X_upper):
            return -np.inf
        if VU_PRINT >= 0:
            print "in ig_mc_freeze, sampling_acquisition_wrapper, : x = ", x, " self.sampling_acquisition = ", self.sampling_acquisition, " np.array[x] = ", np.array([x])
            print "X in model of EI :", self.sampling_acquisition.model.X
            print "self.sampling_acquisition(np.array([x])) = ", self.sampling_acquisition(np.array([x]))

        return self.sampling_acquisition(np.array([x]))[0]

    def update(self, model, calc_repr=False):   #Different
        self.model = model
