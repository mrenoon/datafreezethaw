...
class InformationGainMC(BaseAcquisitionFunction):
    ...
    def compute(self, X=None, overhead=0, derivative=False, *args):
    ...
    def computeOldModel(self, config_id, overhead):
        ...
        config = freezeModel.X[config_id]
        ys = freezeModel.ys[config_id]
        ss = freezeModel.ss[config_id]

        newS = min( ss[-1] + ss[0], 1.0)

        fantOld = freezeModel.predict(option='old', conf_nr=config_id, datasize=newS)[0][0]
        ...
        if VU_PRINT >= 0:
            ...
            print "freezeModel.ss[config_id] = ", freezeModel.ss[config_id]
            print "ss.shape = ", ss.shape, " while ss[config-id].shape = ", freezeModel.ss[config_id].shape
            print "ys.shape = ", ys.shape, " while ys[config-id].shape = ", freezeModel.ys[config_id].shape


        # freezeModel.train(Y=ys, S=ss, do_optimize=False)
        freezeModel.ys[config_id] = freezeModel.y_train[config_id] = ys

        freezeModel.ss[config_id] = freezeModel.s_train[config_id] = ss

        freezeModel.Y[config_id, :] = ys[-1]

        #updated the freezeModel with new fantasized observation

        H_gained_per_time = self.get_H_per_time(freezeModel, newS, overhead, currentS=ss[-1])
        
        if VU_PRINT >= 1:
            print "Infor Gained for size ", newS, " of config #", config_id, " = ", H_gained_per_time

        return H_gained_per_time, newS
    ...
    def get_training_time_old(self, S, overhead, currentS):
        return self.get_training_time(S, overhead) - self.get_training_time(currentS, 0)
