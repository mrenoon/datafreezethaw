...
#            print self.save_file
    ...
    def train(self):
        ...
        print "SELF.epoch = ", self.epoch, " self.batchnum = ", self.batchnum
        ...
        while self.epoch <= self.num_epochs:
            ...
            if self.get_num_batches_done() % self.testing_freq == 0:
                ...
                # print "Test output: ", self.test_outputs[-1]
                # print "len(self.test_outputs) = ", len(self.test_outputs), " len(self.train_batch_range) ", len(self.train_batch_range)
                #VU added
                # if len(self.test_outputs) >= len(self.test_batch_range):
                average_range = min(max(len(self.train_batch_range)/10,3) , 7)
                print "     #### average_range = ", average_range
                if len(self.test_outputs) >= min(2*average_range,8) and self.epoch - startEpoch >= 4*average_range:

                    # print "At least len(test_batch) test outputs, current = ", self.test_outputs[-1][0]['logprob'][1], " previous= ", self.test_outputs[-2][0]['logprob'][1]

                    # average_error = self.aggregate_test_outputs(self.test_outputs[-len(self.test_batch_range):])
                    
                    average_error = self.aggregate_test_outputs(self.test_outputs[-average_range:])
                    
                    print "     #### average error = ", average_error, " = ", average_error[0]['logprob'][1] / average_error[1]
                    # print "test_outputs[last] = ", self.test_outputs[-1]
                    if self.test_outputs[-1][0]['logprob'][1] / self.test_outputs[-1][1] > average_error[0]['logprob'][1] / average_error[1]:
                        print()
                        print("Validation error is greater than the average error !!!")
                        print("STOPPING TRAINING, the best average error was: ")
                        self.print_costs(average_error)
                        # self.print_costs(self.test_outputs[-2])
                        stopEarly = True
                        break

            self.print_elapsed_time(time() - compute_time_py)
        if not stopEarly:
            print "STOPPING TRAINING, done all the specified iterations, the last average error was:"
            average_range = min(max(len(self.train_batch_range)/10,3) , 7)
            average_error = self.aggregate_test_outputs(self.test_outputs[-average_range:])
            self.print_costs(average_error)

    def print_costs(self, dummy):
        pass
